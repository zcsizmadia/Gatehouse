...
`dotnet test` runs every suite against every target framework the libraries claim
to support. TUnit builds each test project into a self-contained executable on
Microsoft.Testing.Platform, which is why it works under NativeAOT and why
Gatehouse uses it; the runner is selected repo-wide by the `test` block in
`global.json`. Running an executable directly also works and is sometimes handier
for one suite:
